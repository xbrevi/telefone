...
@section('corpo')
  ...
  {{-- <!-- RELAÇÃO DE PUBLICADORES ID E NOME --> --}}
      ...
      <tbody>
        <?php foreach($publicadores as $publicador): ?>
            <tr>
                ...
                {{-- <!-- NOME TEXTO DO PUBLICADOR --> --}}
                <td >
                  <div id="nome-{{$publicador->id}}"> 
                    {{ $publicador->nome }}
                  </div>
                </td>
...
<script>
  ...
  function toggleInput(publicadorId) {
    const nomeEl = document.getElementById(`nome-${publicadorId}`);
    const inputEl = document.getElementById(`nome--${publicadorId}`);
  ...
  function editarNome(publicadorId) {
        let formData = new FormData();
        const nome = document.querySelector(`#nome--${publicadorId} > input`).value;
        const token = document.querySelector(`input[name="_token"]`).value;
        const url = `/publicadores/${publicadorId}/edit`;
        ...
        fetch(url, {
            ...
            toggleInput(publicadorId);
            document.getElementById(`nome-${publicadorId}`).textContent = nome;
        });            
